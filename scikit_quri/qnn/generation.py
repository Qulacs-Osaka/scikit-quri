...
class QNNGenerator:
    ...
    def fit_direct_distribution(self,train_scaled:NDArray[np.float64], maxiter: Optional[int] = None) -> Tuple[float, List[float]]:
        ...
        c = 0
        result = minimize(cost_func,theta_init,method="Powell",options={"maxiter":maxiter,"maxfev":maxiter})
        print("")
        print(result)
    ...
    def predict(self) -> NDArray[np.float64]:
        ...
        data_per: NDArray[np.float64] = y_pred_in * y_pred_conj
    ...
    def conving(self, data_diff:NDArray[np.float64]) -> NDArray[np.float64]:
        ...
        #     diff_state
            ...
            raise NotImplementedError(
                f"Kernel type {self.kernel_type} is not implemented yet."
            )
    ...
    def cost_func(
        ...
        cost = abs(np.dot(data_diff, conv_diff))
        print("\r",f"{cost=}",end="")
        return cost
    ...
    def _cost_func_grad(
        ...
        ret = quantum_state(n_qubits=self.n_qubit,vector=convconv_diff*state_vec*4)

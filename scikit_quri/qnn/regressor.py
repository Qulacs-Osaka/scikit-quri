...
class QNNRegressor:
    ...
    def minMaxScaler(
        self, X: NDArray[np.float_], feature_range: tuple[int, int] = (0, 1)
    ):
        """
    ...
    def cost_func(
        ...
        y_pred = self._predict_inner(x_scaled, theta)
        cost = mean_squared_error(y_pred, y_scaled)
        return cost

    # !miniBatch処理未実装
    def fit(
        self, x_train: NDArray[np.float_], y_train: NDArray[np.float_], batch_size=32
    ) -> None:
        """
        ...
        if self.do_x_scale:
            x_scaled = self.minMaxScaler(
                x_train, feature_range=(-self.x_norm_range, self.x_norm_range)
            )
        else:
            x_scaled = x_train

        if self.do_y_scale:
            y_scaled = self.minMaxScaler(
                y_train, feature_range=(-self.y_norm_range, self.y_norm_range)
            )
        else:
            y_scaled = y_train
        ...
        for x_batched, y_batched in zip(x_split, y_spilt):
            ...
            result = minimize(
                self.cost_func,
                init_params,
                args=(x_batched, y_batched),
                # method="Nelder-Mead",
                method="COBYLA",
                options={"maxiter": maxiter},
                callback=self.callback,
            )
            # optimizer_state.cost = result.fun
            self.trained_param = result.x
    ...
    def cost_fn(
        self,
        x_batched: NDArray[np.float_],
        y_batched: NDArray[np.float_],
    ...
    def predict(self, x_test: NDArray[np.float_]) -> NDArray[np.float_]:
        ...
        if self.do_x_scale:
            x_scaled = self.minMaxScaler(
                x_test, feature_range=(-self.x_norm_range, self.x_norm_range)
            )
        else:
            x_scaled = x_test
    ...
    # * 未使用
    def _embed_x_circuit(
        self, x_scaled: NDArray[np.float_]
    ) -> List[ParametricCircuitQuantumState]:
        circuits = []
        for x in x_scaled:
    ...
    def _predict_inner(
        ...
        if isinstance(self.ansatz, LearningCircuit):
            for x in x_scaled:
                ...
                parametric_circuit = ParametricCircuitQuantumState(
                    self.n_qubits, self.ansatz.circuit
                )
                # if res == []:
                #     for gate in c.gates:
                #         print(f"name:{gate.name} params:{gate.params}")
                estimate = self.estimator(
                    self.operator, parametric_circuit, circuit_params
                )
                # ? これはout_dim == 1のみの応急処理

...
def create_farhi_neven_ansatz(n_qubit: int, c_depth: int):
    ...
    for i in range(n_qubit):
        linear_param_circuit.add_ParametricRY_gate(i, {theta: 1.0})
        linear_param_circuit.add_ParametricRZ_gate(
...
if __name__ == "__main__":
    ...
    # op.add_term(pauli_label("Z0"), 2.0)
    ...
    # Create Instance
    qnn = QNNRegressor(
        n_qubits, parametric_circuit, estimator, gradient_estimator, adam, op
    )
    ...
    plt.plot(np.sort(x_test), y_test[np.argsort(x_test.flatten())], label="Test")

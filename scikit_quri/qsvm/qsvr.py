...
class QSVR:
    ...
    def _run_circuit(self, x: NDArray[np.float64]):
        """
    ...
    def fit(self, x: NDArray[np.float64], y: NDArray[np.float64], n_shots: int = 1000, sampling_backend: Optional[SamplingBackend] = None):
        """
        Train the machine.
        
        Parameters: 
            x: Training inputs.
            y: Training teacher values.
    ...
    def predict(self, xs: NDArray[np.float64]) -> NDArray[np.float64]:
        ...
        Parameters:
            xs: Input to make predictions
        
        Returns:
            pred: Predicted values of y.
        """
